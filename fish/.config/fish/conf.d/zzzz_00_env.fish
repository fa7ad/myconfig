...
set -l py_paths $HOME/Library/Python/*/bin
set -l go_paths $HOME/go/bin
set -l bun_path $BUN_INSTALL/bin
set -l cargo_path $HOME/.cargo/bin

set -l unprocessed_path (bash -c "echo -n $PATH" | string split ':') $local_paths $go_paths $yarn_paths $py_paths $bun_path $cargo_path
