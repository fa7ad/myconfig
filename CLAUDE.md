...
- `scripts/.bin/` — personal scripts/binaries added to `PATH`; includes the `i3-sensible-app` git submodule (Linux-era, likely vestigial on macOS).
...
- This repo predates the current macOS-only setup (see README.md) — some Linux-era files/submodules may be vestigial; check before assuming a config path is still in use.
